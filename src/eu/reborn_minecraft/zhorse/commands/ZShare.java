...
public class ZShare {
	...
	public ZShare(CommandSender s, String[] a, ZHorse zh) {
								...
								if (zh.getEM().isReadyToPay(p, a[0])) {
									...
									if (!zh.getUM().isShared(horse)) {
										if (zh.getUM().isLocked(horse)) {
											zh.getUM().lock(zh.getUM().getPlayerUUID(horse), horse, false);
											p.sendMessage(String.format(zh.getLM().getCommandAnswer(zh.getLM().horseUnLocked), horseName));
										}
										zh.getUM().share(zh.getUM().getPlayerUUID(horse), horse, true);
										p.sendMessage(String.format(zh.getLM().getCommandAnswer(zh.getLM().horseShared), horseName));
									}
									else {
										zh.getUM().share(zh.getUM().getPlayerUUID(horse), horse, false);
										p.sendMessage(String.format(zh.getLM().getCommandAnswer(zh.getLM().horseUnShared), horseName));
									}
