...
public class ZLock {
	...
	public ZLock(CommandSender s, String[] a, ZHorse zh) {
							...
							if (zh.getUM().isClaimedBy(p.getUniqueId(), horse) || zh.getPerms().has(p, perm + zh.getLM().bypass)) {
								if (zh.getEM().isReadyToPay(p, a[0])) {
									...
									if (!zh.getUM().isLocked(horse)) {
										...
										zh.getUM().lock(zh.getUM().getPlayerUUID(horse), horse, true);
										p.sendMessage(String.format(zh.getLM().getCommandAnswer(zh.getLM().horseLocked), horseName));
									}
									else {
