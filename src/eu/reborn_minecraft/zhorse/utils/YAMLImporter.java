...
public class YAMLImporter {
	...
	private static boolean importPlayer(ZHorse zh, FileConfiguration db, String playerUUID) {
		if (zh.getDM().playerExists(playerUUID)) {
			return true;
		}
		...
		return zh.getDM().insertPlayer(playerUUID, playerName, language, favorite);
	}
	...
	private static boolean importHorses(ZHorse zh, FileConfiguration db, String playerUUID) {
		boolean success = true;
		...
		if (cs != null) {
			for (String horseID : cs.getKeys(false)) {
	...
	private static boolean importHorse(ZHorse zh, FileConfiguration db, String ownerUUID, String horseID) {
		String horseUUID = getHorseStringData(db, ownerUUID, horseID, KeyWordEnum.uuid.getValue(), null);
		if (zh.getDM().horseExists(horseUUID)) {
			return true;
		}
		int horseIDInt = Integer.parseInt(horseID);
		...
		return zh.getDM().insertHorse(horseUUID, ownerUUID, horseIDInt, horseName, modeLocked, modeProtected, modeShared, locationWorld, locationX, locationY, locationZ);
	}
	
	private static boolean getHorseBooleanData(FileConfiguration db, String playerUUID, String horseID, String valuePath, boolean defaultValue) {
		String horsesPath = KeyWordEnum.players.getValue() + KeyWordEnum.dot.getValue() + playerUUID + KeyWordEnum.dot.getValue() + KeyWordEnum.horses.getValue();
	...
	private static String getHorseStringData(FileConfiguration db, String playerUUID, String horseID, String valuePath, String defaultValue) {
		String horsesPath = KeyWordEnum.players.getValue() + KeyWordEnum.dot.getValue() + playerUUID + KeyWordEnum.dot.getValue() + KeyWordEnum.horses.getValue();
		String dataPath = horsesPath + KeyWordEnum.dot.getValue() + horseID + KeyWordEnum.dot.getValue() + valuePath;
		return db.getString(dataPath, defaultValue);
	}
